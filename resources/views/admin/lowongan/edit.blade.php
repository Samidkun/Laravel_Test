...
<div class="max-w-2xl mx-auto space-y-6">
    <!-- Header -->
    <div class="space-y-1">
        <h1 class="text-xl md:text-2xl font-bold text-slate-900">Edit Lowongan Magang</h1>
        <p class="text-xs text-slate-500">Perbarui kuota, divisi, atau rincian deskripsi lowongan ini.</p>
    </div>

    <!-- Form Card -->
    <div class="bg-white border border-slate-200 rounded-lg p-6 shadow-sm card-primary">
        <form action="{{ route('admin.lowongan.update', $lowongan->id) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                <div class="sm:col-span-2 space-y-1">
                    ...
                    <input type="text" id="posisi" name="posisi" value="{{ old('posisi', $lowongan->posisi) }}" class="w-full px-3 py-2 bg-white border border-slate-350 border-slate-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-lg text-sm text-slate-800 transition-colors">
                    @error('posisi')
                        <span class="text-xs text-rose-600 block mt-1">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <input type="number" id="quota" name="quota" value="{{ old('quota', $lowongan->quota) }}" min="1" class="w-full px-3 py-2 bg-white border border-slate-350 border-slate-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-lg text-sm text-slate-800 transition-colors">
                    @error('quota')
                        <span class="text-xs text-rose-600 block mt-1">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="space-y-1">
                ...
                <textarea id="deskripsi" name="deskripsi" rows="5" class="w-full px-3 py-2 bg-white border border-slate-350 border-slate-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 rounded-lg text-sm text-slate-800 transition-colors">{{ old('deskripsi', $lowongan->deskripsi) }}</textarea>
                @error('deskripsi')
                    <span class="text-xs text-rose-600 block mt-1">{{ $message }}</span>
                @enderror
            </div>
